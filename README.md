...
3. Replace the placeholder images. In WebsitePublisher, use the visual editor: every image has a unique `data-wpe-slot`.
...
## WebsitePublisher structure

The published version uses reusable `site-header` and `site-footer` fragments, while the local version injects the same shared elements through `js/site.js`. Optimizer comment tags are already included on every page for canonical, Open Graph, and tracking support.
